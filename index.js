...
button.addEventListener("click", () => {
  ...
  if (
    parseInt(totalRange.value) > allowed &&
    excludeDuplicates &&
    allowed > 0
  ) {
    ...
    passwordField.setAttribute(
      "placeholder",
      "You have reached maximum range !"
    );
    noPassword = true;
  } else {
  ...
  if (lower || upper || symbolsBool || number) {
    while (length > 0) {
      ...
      if (op == 1 && lower === true) {
        ...
        password += letter;
        if (excludeDuplicates) lowerLetter = lowerLetter.replace(letter, "");
        length--;
        continue;
      }
      if (op == 2 && upper === true) {
        ...
        password += letter;
        if (excludeDuplicates) upperLetters = upperLetters.replace(letter, "");
        length--;
      }
      if (op == 3 && symbolsBool === true) {
        let letter = randomElement(symbols);
        password += letter;
        if (excludeDuplicates) symbols = symbols.replace(letter, "");
      ...
      if (op == 4 && number === true) {
        ...
        password += letter;
        if (excludeDuplicates) numbers = numbers.replace(letter, "");
        length--;
      }
    }
    return password;
